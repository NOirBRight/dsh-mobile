...
- **官方功能、手机构图** — 窄屏变成顶栏、会话单栏、导航抽屉和 rightbar 面。宽屏仍用官方桌面布局。
...
请在同一台 Host 上安装 **[Codex Sidebar](https://github.com/NOirBRight/dsh-codex-sidebar)**（命令见[安装](#安装)）。桌面上它占用官方 rightbar 列；手机上这个席位变成右侧抽屉。Files、Review、Browser、Terminal 仍是同一套插件，不是手机另做一版。
