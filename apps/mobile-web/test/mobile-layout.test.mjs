...
test('narrow layout lets official StatsPills occupy the composer dock', async () => {
  ...
  assert.match(css, /\[class\*="headerUtilities"\]/, 'mobile header must target the real utility class')
  assert.match(css, /div:has\(> \[data-sidebar-right-expand\]\)/, 'corner adaptation must use the public rightbar control')
  assert.match(css, /\[data-slot="conversation\.composer\.dock"\] > \*/, 'mobile stats must target the official composer dock')
  assert.match(css, /data-mobile-stats-pills/, 'interactive stats keep their own mobile seam')
...
test('mobile drawer closes on navigation and reports its constrained rendered width', async () => {
  ...
  try {
    ...
    assert.equal(capture('pills-fit'), 'true', 'interactive statistics keep readable text, icons, alignment and equal gaps without clipping')
    ...
    assert.ok(Math.abs(Number(capture('header-menu-delta'))) <= 16, 'Chat should start beside the hamburger: ' + capture('header-menu-delta'))
    assert.ok(Number(capture('header-right-inset')) <= 40, 'row-2 right inset: ' + capture('header-right-inset'))
    assert.equal(capture('download-after-jobs'), 'true', 'download control must follow Jobs in the compact header')
    assert.ok(Number(capture('download-right-inset')) <= 40, 'download control must stay on the far right: ' + capture('download-right-inset'))
