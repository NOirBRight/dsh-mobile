...
test('mobile settings uses a full-width panel and horizontal category navigation', async () => {
  ...
  try {
    ...
    assert.equal(capture('close-width'), '24', 'settings close action should use a compact hit target')
    assert.equal(capture('close-height'), '24', 'settings close action should use a compact hit target')
