...
test('mobile settings uses a full-width panel and horizontal category navigation', async () => {
  ...
  try {
    ...
    assert.equal(capture('overlay-padding-top'), '40px', 'settings overlay should reserve notification-bar clearance')
    assert.equal(capture('overlay-padding-bottom'), '12px', 'settings overlay should retain bottom breathing room')
    assert.ok(Number(capture('panel-top')) >= 40, 'settings panel should stay below the notification-bar clearance')
    assert.equal(capture('panel-nav-top'), '0', 'settings navigation should start at the panel edge without an internal blank strip')
    assert.equal(capture('panel-width'), String(viewportWidth - 24), 'mobile settings panel should preserve usable side margins')
    assert.equal(capture('panel-height'), String(viewportHeight - 52), 'mobile settings panel should account for top and bottom clearance')
    ...
    assert.equal(capture('close-width'), '40', 'settings close action should use a touch-safe hit target')
    assert.equal(capture('close-height'), '40', 'settings close action should use a touch-safe hit target')
    assert.match(capture('close-border'), /^0px/, 'settings close action should not add an unexplained outer ring')
    assert.equal(capture('close-background'), 'rgba(0, 0, 0, 0)', 'settings close action should stay visually quiet')
