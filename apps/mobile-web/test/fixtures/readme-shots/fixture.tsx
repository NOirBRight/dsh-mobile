...
function Frame({ drawerOpen, detailsOpen }: { drawerOpen: boolean; detailsOpen: boolean }) {
  const panels = { drawerOpen, rightbarOpen: detailsOpen, panelInfo: { activePanelId: null } }
  ...
  const renderSlot = (name: string) => {
    ...
    if (name === 'main' || name === 'conversation') return <Conversation />
    if (name === 'rightbar' || name === 'details') return <CodexFiles />
