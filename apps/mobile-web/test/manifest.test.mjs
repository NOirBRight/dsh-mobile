...
test('replaces desktop layout and drops browser HMR without mutating host manifest', () => {
  ...
  assert.equal(mobile.rev, 'host-rev+mobile-layout-0.1.60+mobile-interactions-0.1.17')
  ...
  assert.deepEqual(mobile.entries[1], {
    id: MOBILE_LAYOUT_ID,
    url: '/plugins/@dsh-mobile/ui-layout-mobile/client.js?rev=0.1.60',
    rev: '0.1.60',
    inject: [
...
test('localizes host plugin scripts while keeping the packaged mobile layout', async () => {
  const manifest = {
    ...
    entries: [
      ...
      { id: MOBILE_LAYOUT_ID, url: '/plugins/@dsh-mobile/ui-layout-mobile/client.js?rev=0.1.60', rev: '0.1.60', inject: [] },
  ...
  assert.deepEqual(manifest.entries.map(entry => entry.url), [
    '/plugins/runtime/client.js?rev=a',
    '/plugins/@dsh-mobile/ui-layout-mobile/client.js?rev=0.1.60',
