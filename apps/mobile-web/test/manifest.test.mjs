...
test('replaces desktop layout and drops browser HMR without mutating host manifest', () => {
  ...
  assert.equal(mobile.rev, 'host-rev+mobile-layout-0.1.30+mobile-interactions-0.1.8')
...
test('localizes host plugin scripts while keeping the packaged mobile layout', async () => {
  const manifest = {
    ...
    entries: [
      ...
      { id: INTERACTION_OPERATIONS_ID, url: '/plugins/@dsh-mobile/interaction-operations/client.js?rev=0.1.8', rev: '0.1.8', inject: [] },
  ...
  assert.deepEqual(manifest.entries.map(entry => entry.url), [
    ...
    '/plugins/@dsh-mobile/interaction-operations/client.js?rev=0.1.8',
