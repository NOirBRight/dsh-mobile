...
test('loopback Gateway exposes bounded HTTP and signaling/tunnel WebSockets', async (t) => {
  ...
  const secondSignal = new WebSocket('ws://127.0.0.1:' + port + '/signal/' + room)
  await Promise.all([once(signal, 'open'), once(secondSignal, 'open')])
  assert.equal(signal.readyState, WebSocket.OPEN)
  signal.send('offer-a'); secondSignal.send('offer-b')
  assert.equal(String((await once(signal, 'message'))[0]), 'answer:offer-a')
  assert.equal(String((await once(secondSignal, 'message'))[0]), 'answer:offer-b')
  signal.close(); secondSignal.close()

  const firstTunnel = new WebSocket('ws://127.0.0.1:' + port + '/tunnel/' + room)
  const secondTunnel = new WebSocket('ws://127.0.0.1:' + port + '/tunnel/' + room)
  await Promise.all([once(firstTunnel, 'open'), once(secondTunnel, 'open')])
  assert.equal(firstTunnel.readyState, WebSocket.OPEN)
  const firstMessage = once(firstTunnel, 'message')
  const secondMessage = once(secondTunnel, 'message')
  firstTunnel.send(Buffer.from([1, 2, 3]))
  secondTunnel.send(Buffer.from([4, 5, 6]))
  assert.deepEqual(Buffer.from((await firstMessage)[0]), Buffer.from([1, 2, 3]))
  assert.deepEqual(Buffer.from((await secondMessage)[0]), Buffer.from([4, 5, 6]))
  firstTunnel.close(); secondTunnel.close()
