...
9. Compact rendering belongs inside the official module that owns the content, preferably through container-aware presentation. The mobile root must not depend on deep global DOM selectors or generated CSS-module class names to rewrite token statistics, per-turn metrics, conversation headers, or composer controls. Exception: Host MenuView has no public decorate seam for `/` skill rows or plugin commands that omit `icon`. The narrow layout may wrap `inputTriggers` skill sources and stamp a cube onto icon-less `[role=option]` nodes after MenuView commits. It must not wrap the Host `command` source or replace an official SVG already present.
10. Inside the APK/mobile shell, responsive behavior is width-driven: wide viewports keep the official root, narrow viewports use `@dsh-mobile/ui-layout-mobile`. Ordinary Host WebUI stays the official layout and must not load this plugin.
