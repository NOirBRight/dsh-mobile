...
DSH Mobile has no project-operated Relay, Discovery Service, TURN server, runtime CDN, or maintainer-domain dependency. One Host process owns a loopback-only Gateway; the chosen HTTPS/WebSocket Public Endpoint forwards only to that Gateway. The Gateway serves the packaged browser shell plus bounded protocol routes and forwards authenticated DSH traffic only to the configured loopback DSH web port.
...
`npm run build` copies `apps/mobile-web/dist` into `$DSH_HOME/mobile/browser-shell` when `DSH_HOME` or `DSH_MOBILE_BROWSER_SHELL` is set. Without either variable the Host-side copy is skipped.
...
- id: dsh-mobile-pairing
  ...
  config:
    ...
    gatewayPort: 0
    browserShellPath: /absolute/DSH_HOME/mobile/browser-shell
...
The Host settings page is `http://127.0.0.1:3080/pair/ui`. It shows the current endpoint and Host Identity, produces separate Android/browser QR offers, lists authorized devices, creates room-preserving Endpoint Refresh offers, and performs Host-side revocation. The Public Endpoint itself does **not** expose `/pair`, token exchange, credential minting, generic proxy targets, or raw DSH port 3080.

## Connection and lifecycle behavior

- Automatic tries WebRTC RTCDataChannel first and uses encrypted Tunnel Fallback only after a connectivity failure. Direct Only and Tunnel Only are per-Host Profile options.
- Authorization errors and Host Identity mismatches are terminal; they never trigger hidden fallback.
- Temporary Endpoint hostname rotation requires a new Endpoint Refresh QR. Native clients retain the existing Host Profile and device token because Host Identity, not hostname, is authoritative. Browser clients re-pair after Quick Tunnel hostname rotation because browser storage is origin scoped.
