...
Android APK ── wss://pair.noirbright.top ──> VPS: Docker Caddy :443 (Let's Encrypt)
                                             ...
                                             AM01S: Host Gateway 127.0.0.1:43168 (pairing plugin on daily 3080)
                                               │ loopback only
...
- The return path is the restricted SSH reverse tunnel unit `dsh-gateway-vps-tunnel.service` (`ssh -N -R 172.18.0.1:31424:127.0.0.1:43168 vps-aliyun`). The public Pair endpoint intentionally terminates at the daily 3080 Host; this is not a lab/3082 mismatch.
- AM01S `dsh-pair-mux.service` runs the installed published `dsh-pair-mux` executable through its configured `PATH`. Install `dsh-mobile-pairing#v0.1.11` before enabling the unit and verify it with `command -v dsh-pair-mux`.
...
1. **The public Pair mapping is intentionally the daily 3080 Host.** `pair.noirbright.top` returns the daily Host Identity and forwards to Gateway 43168. Do not redirect it to the lab 3082 process. The lab 3082 endpoint can be tested separately with its own custom URL/Relay selection.
2. **The daily DSH is the public Pair owner.** Keep `dsh-web.service`/daily 3080 and Gateway 43168 healthy for `pair.noirbright.top`. The lab 3082/43169 process remains an independent test plane and must not claim the daily Pair DNS.
...
4. **Fixed ports are required end to end.** The daily Pair Gateway must keep `gatewayPort: 43168` (never the default `0`), matching the `-R` forward and the Caddy site block. Lab test Gateway 43169 remains separate.
